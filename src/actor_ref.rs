...
/// by Hewitt).
pub struct ActorRef<Args: Copy  + Send + 'static, M: Copy  + Send + 'static + Reflect, A: Actor<M> + 'static> {
    actor_cell: ActorCell<Args, M, A>,
}

impl<Args: Copy  + Send + 'static, M: Copy  + Send + 'static + Reflect, A: Actor<M> + 'static> Clone for ActorRef<Args, M, A> {
    fn clone(&self) -> ActorRef<Args, M, A> {
        ActorRef::with_cell(self.actor_cell.clone())
    }
}

impl<Args: Copy  + Send + 'static, M: Copy  + Send + 'static + Reflect, A: Actor<M> + 'static> ActorRef<Args, M, A> {
...
impl<Args: Copy  + Send + 'static, M: Copy  + Send + 'static + Reflect, A: Actor<M> + 'static> CanReceive for ActorRef<Args, M, A> {
    fn receive(&self, message: Box<Any>, sender: Arc<CanReceive>) {
